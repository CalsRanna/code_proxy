...
void main() {
  group('ProxyServerRouter', () {
    test('失败响应应统一走断路器重试与故障转移', () async {
      ...
      expect(session.currentEndpoint, isNotNull);
      expect(session.currentEndpoint?.id, 'ep-1');

      session.recordFailure();
      expect(await session.advanceAfterAttempt(), isTrue);
      expect(session.currentEndpoint?.id, 'ep-1');
    ...
    test('open 端点在恢复超时后应以 halfOpen 重新参与选择', () async {
      ...
      expect(firstSession.currentEndpoint, isNotNull);
      firstSession.recordFailure();
      expect(await firstSession.advanceAfterAttempt(), isFalse);
      expect(
        registry.getBreaker('ep-1').state,
        ProxyServerCircuitBreakerState.open,
      );
      ...
      expect(recoverySession.currentEndpoint, isNotNull);
      expect(
        registry.getBreaker('ep-1').evaluateState(),
        ProxyServerCircuitBreakerState.halfOpen,
      );

      recoverySession.recordSuccess();
    ...
    test('并发请求失败时不应把断路器错误应用到其他端点', () async {
      ...
      expect(sessionA.currentEndpoint, isNotNull);
      expect(sessionA.currentEndpoint?.id, 'ep-1');
      expect(sessionB.currentEndpoint, isNotNull);
      expect(sessionB.currentEndpoint?.id, 'ep-1');

      sessionA.recordFailure();
      expect(await sessionA.advanceAfterAttempt(), isTrue);
      ...
      sessionB.recordFailure();
      expect(await sessionB.advanceAfterAttempt(), isTrue);
