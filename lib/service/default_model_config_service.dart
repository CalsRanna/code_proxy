...
class DefaultModelConfigService {
  static final instance = DefaultModelConfigService._();
  DefaultModelConfigService._();

  DefaultModelConfig? _config;

  /// 获取配置
  ///
  /// 如果配置未加载或加载失败，抛出异常
  DefaultModelConfig get config {
    if (_config == null) {
  ...
  Future<void> load() async {
    ...
    if (!file.existsSync()) {
      ...
      _config = DefaultModelConfig.defaultConfig;
      return;
    }
    ...
    try {
      ...
      if (detectLegacyKeys(yaml)) {
        ...
        _config = DefaultModelConfig.fromYaml(yaml);
      }
  ...
  /// 容忍为空。写回文件由调用方完成。
  @visibleForTesting
  static DefaultModelConfig migrateLegacyConfig(YamlMap yaml) {
  ...
  Future<void> _createDefaultConfig(File file) async {
    await file.parent.create(recursive: true);
    await file.writeAsString(DefaultModelConfig.defaultConfig.toYamlString());
  }

  @visibleForTesting
  void replaceConfigForTesting(DefaultModelConfig config) {
    _config = config;
  }
}
