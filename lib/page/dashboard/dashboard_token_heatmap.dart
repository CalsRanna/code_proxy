...
class DashboardTokenHeatmap extends StatefulWidget {
...
class _DashboardTokenHeatmapState extends State<DashboardTokenHeatmap> {
  ...
  Widget _buildTooltip(List<List<_DayData>> heatmapData, double cellSize) {
    ...
    final stride = cellSize + DashboardTokenHeatmap._cellSpacing;
    final cellLeft = DashboardTokenHeatmap._weekdayLabelWidth +
        ShadcnSpacing.spacing4 +
        week * stride +
        DashboardTokenHeatmap._cellMargin;
    final cellTop = _cellsTopOffset + day * stride + DashboardTokenHeatmap._cellMargin;

    // 与图表 tooltip 一致的样式：白色加粗标题行 + 紫色副行；
    // 日期不展示年份
    final shortDate = '${dayData.date.month.toString().padLeft(2, '0')}-'
  ...
  void _handleHover(
    ...
    final stride = cellSize + DashboardTokenHeatmap._cellSpacing;
    final x = localPosition.dx - DashboardTokenHeatmap._weekdayLabelWidth -
        ShadcnSpacing.spacing4;
    ...
    if (week < 0 ||
        ...
        day < 0 ||
        day >= DashboardTokenHeatmap._daysPerWeek) {
      _clearHoverIfNeeded();
      return;
    }
    final weekData = heatmapData[week];
  ...
  static const double _cellsTopOffset =
      DashboardTokenHeatmap._monthLabelHeight + ShadcnSpacing.spacing4;

  /// 格子行起点 x：星期标签列 + 右侧间距。
  static const double _cellsLeftOffset =
      DashboardTokenHeatmap._weekdayLabelWidth + ShadcnSpacing.spacing4;

  double _totalWidth(int totalWeeks, double cellSize) {
    return totalWeeks * (cellSize + DashboardTokenHeatmap._cellSpacing) +
        _cellsLeftOffset;
  }

  double _totalHeight(double cellSize) {
    return _cellsTopOffset +
        DashboardTokenHeatmap._daysPerWeek *
            (cellSize + DashboardTokenHeatmap._cellSpacing);
  }
  ...
  double _calculateCellSize(double availableWidth, int totalWeeks) {
    ...
    return (adjustedWidth / totalWeeks) - DashboardTokenHeatmap._cellSpacing;
  }

  /// 计算包含指定日期的第一个周日
  DateTime _calculateFirstSunday(DateTime date) {
    final daysFromSunday = date.weekday % DashboardTokenHeatmap._daysPerWeek;
  ...
  DateTime _calculateLastSaturday(DateTime date) {
    final daysToSaturday =
        (6 - date.weekday % DashboardTokenHeatmap._daysPerWeek);
  ...
  void _fillRemainingWeek(List<_DayData> week) {
    while (week.length < DashboardTokenHeatmap._daysPerWeek) {
  ...
  List<List<_DayData>> _generateWeeksList(
    ...
    for (int i = 0; i < totalDays; i++) {
      ...
      if (currentWeek.length == DashboardTokenHeatmap._daysPerWeek) {
...
class _HeatmapPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final stride = cellSize + DashboardTokenHeatmap._cellSpacing;
    final cellsTop = _DashboardTokenHeatmapState._cellsTopOffset;
    final cellsLeft = _DashboardTokenHeatmapState._cellsLeftOffset;
    ...
    for (int i = 0; i < weekdays.length; i++) {
      ...
      final textPainter = TextPainter(
        text: TextSpan(
          text: label,
          style: const TextStyle(
            fontSize: DashboardTokenHeatmap._weekdayLabelFontSize,
      ...
      textPainter.paint(
        canvas,
        Offset(
          (DashboardTokenHeatmap._weekdayLabelWidth - textPainter.width) / 2,
          y,
        ),
      );
    }
  ...
  void _paintCells(
    ...
    final margin = DashboardTokenHeatmap._cellMargin;
      ...
      for (int day = 0; day < weekData.length; day++) {
        ...
        canvas.drawRRect(
          RRect.fromRectAndRadius(
            rect,
            const Radius.circular(DashboardTokenHeatmap._borderRadius),
          ),
          paint,
        );
      }
        ...
        if (month != lastMonth) {
          final textPainter = TextPainter(
            text: TextSpan(
              text: '$month',
              style: const TextStyle(
                fontSize: DashboardTokenHeatmap._monthLabelFontSize,
                color: ShadcnColors.zinc500,
              ),
            ),
            textDirection: TextDirection.ltr,
          ...
          textPainter.paint(
            ...
            Offset(
              ...
              (DashboardTokenHeatmap._monthLabelHeight - textPainter.height) /
                  2,
            ),
          );
          lastMonth = month;
