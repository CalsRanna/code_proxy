...
- **智能重试** — 5xx 错误和网络异常自动重试，采用全抖动指数退避：每次随机等待 0 到当前上限，上限依次为 1、2、4、8、16、32 秒，此后每次仍在 0～32 秒间随机等待
- **断路器保护** — 连续失败达到阈值后熔断端点，仅 5xx 错误和网络异常计入断路器，4xx 错误直接返回客户端
...
| 配置 | 默认值 | 说明 |
|------|--------|------|
| 持续重试模式 | 关闭 | 同一端点在 API 超时内持续重试所有上游 4xx、5xx 和网络错误，不自动故障转移 |
| 端点熔断阈值 | 5 | 连续失败次数达到此值后禁用端点并故障转移 |
| 端点恢复超时 | 60 秒 | 端点被禁用后等待此时间再尝试探测恢复 |
| API 超时 | 10 分钟 | 单次请求的超时时间 |
| 审计日志保留天数 | 14 天 | 超期自动清理 |
| 开机自启 | 关闭 | 系统启动时自动运行 |

持续重试模式可在「设置 → 代理服务器」随时开启或关闭。切换会终止当前全部代理请求和重试（包括 SSE），立即应用新模式；监听地址、端口、认证令牌及客户端配置保持不变，无需重启代理。开启后，「端点熔断阈值」和「端点恢复超时」置灰并禁止编辑，原有描述和配置值保留；关闭后恢复可编辑。

启用后，每个请求固定使用优先级最高的已启用端点，重试没有次数上限，与正常模式一样采用全抖动指数退避：每次随机等待 0 到当前上限，上限依次为 1、2、4、8、16、32 秒，此后每次仍在 0～32 秒间重新随机等待；上游有效的 `Retry-After` 比抽样结果更长时按其要求等待。请求构造、上游连接、错误体读取和重试等待共用一个 API 超时预算，耗尽后返回 504。成功进入 SSE 后继续按原有空闲超时输出，不因重试窗口到期而中断；手动切换模式仍会终止该流。此模式可能重复发送上游已执行的请求并产生额外计费，仅用于特殊端点。

持续重试模式下，失败请求（包括重试过程中的失败、最终超时和流式中断）不写入请求数据库，也不生成关联审计文件；成功请求照常记录。运行日志仍保留重试诊断信息。关闭此模式后，失败请求恢复正常记录。

客户端断开连接后，代理会取消该请求的重试等待并关闭其上游连接，运行日志显示 `reason=Client disconnected`。客户端超时后自动重发属于新的 HTTP 请求，会产生新的请求 ID；只要旧连接已断开，旧重试链就会结束。
